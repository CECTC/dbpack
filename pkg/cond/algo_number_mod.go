...
func (shard *NumberMod) Topology() *topo.Topology {
	return shard.topology
}

func (shard *NumberMod) Equal(algorithm ShardingAlgorithm) bool {
	if algo, ok := algorithm.(*NumberMod); ok {
		if algo.topology.Equal(algo.topology) {
			return true
		}
	}
	return false
