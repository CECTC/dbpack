...
func TestPrepareUpdate(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.sql, func(t *testing.T) {
			...
			updateColumns := executor.(*prepareUpdateExecutor).GetUpdateColumns()
			assert.Equal(t, c.expectedUpdateColumns, updateColumns)
			beforeImageSql := executor.(*prepareUpdateExecutor).buildBeforeImageSql(tableMeta)
			assert.Equal(t, c.expectedBeforeImageSql, beforeImageSql)
			afterImageSql := executor.(*prepareUpdateExecutor).buildAfterImageSql(tableMeta)
