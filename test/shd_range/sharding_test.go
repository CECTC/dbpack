...
func (suite *_ShardingSuite) TestExprShadow() {
	for i := 0; i < 20; i++ {
		_, err := suite.db.Exec(insertCityShadow, 10+i*10, fmt.Sprintf("city%d", i), "US", fmt.Sprintf("district%d", i), 100000)
		suite.Assert().Nil(err)
	}
...
func (suite *_ShardingSuite) TestHintShadow() {
	for i := 0; i < 20; i++ {
		_, err := suite.db.Exec(insertCityHintShadow, 210+i*10, fmt.Sprintf("city%d", i), "US", fmt.Sprintf("district%d", i), 100000)
		suite.Assert().Nil(err)
	}
	rows, err := suite.db.Query(selectCityShadow)
	if suite.NoErrorf(err, "select row error: %v", err) {
